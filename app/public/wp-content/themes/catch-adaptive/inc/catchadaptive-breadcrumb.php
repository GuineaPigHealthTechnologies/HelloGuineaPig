 ...
 * Add breadcrumb.
 *
 * @action catchadaptive_after_header
 *
 * @since Catch Adaptive 0.1
 */
if ( !function_exists( 'catchadaptive_add_breadcrumb' ) ) :
...
add_action( 'catchadaptive_before_content', 'catchadaptive_add_breadcrumb', 40 );
 ...
 * @since Catch Adaptive 0.1
 */
if ( !function_exists( 'catchadaptive_custom_breadcrumbs' ) ) :

	function catchadaptive_custom_breadcrumbs( $showOnHome, $delimiter ) {

		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'catch-adaptive' ); // text for the 'Home' link
		$text['category'] = __( '%1$s Archive for %2$s', 'catch-adaptive' ); // text for a category page
		$text['search']   = __( '%1$sSearch results for: %2$s', 'catch-adaptive' ); // text for a search results page
		$text['tag']      = __( '%1$sPosts tagged %2$s', 'catch-adaptive' ); // text for a tag page
		$text['author']   = __( '%1$sView all posts by %2$s', 'catch-adaptive' ); // text for an author page
		$text['404']      = __( 'Error 404', 'catch-adaptive' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( __( 'Page %s', 'catch-adaptive' ), max( $paged, $page ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
	...
	} // end catchadaptive_breadcrumb_lists
endif;
